...
{
	PC = 0; pPC = 0; addr = 0; storadd = 0;
	A = 0; T = 0; SP = 0xFF;
	X = 0; Y = 0; IR = 0; ST = 0x00;  
	cycles = 0;
}
...
{ //'IR' is the instruction-register, naming after the hardware-equivalent
	IR = Ram.memory[PC]; cycles = 2; storadd = 0; //'cycle': ensure smallest 6510 runtime (for implied/register instructions)
	if (IR & 1) {  //nybble2:  1/5/9/D:accu.instructions, 3/7/B/F:illegal opcodes
		...
		case 1: case 3: PC++; addr = Ram.memory[Ram.memory[PC] + X] + Ram.memory[Ram.memory[PC] + X + 1] * 256; cycles = 6; break; //(zp,x)
		...
		switch (IR & 0xF0) {
		case 0x60: SP++; SP &= 0xFF; A = Ram.memory[0x100 + SP]; ST &= 125;ST |= (!A) << 1 | (A & 128); cycles = 4; break; //PLA
		...
		case 0x00: Ram.memory[0x100 + SP] = ST; SP--; SP &= 0xFF; cycles = 3; break; //PHP
		case 0x20: SP++; SP &= 0xFF; ST = Ram.memory[0x100 + SP]; cycles = 4; break; //PLP
		case 0x40: Ram.memory[0x100 + SP] = A; SP--; SP &= 0xFF; cycles = 3; break; //PHA
		...
		if ((IR & 0x1F) == 0x10) {
			PC++; T = Ram.memory[PC]; if (T & 0x80) T -= 0x100; //BPL/BMI/BVC/BVS/BCC/BCS/BNE/BEQ  relative branch 
			...
			addr &= 0xFFFF;
			switch (IR & 0xE0) {
			case 0x00: Ram.memory[0x100 + SP] = PC % 256; SP--;SP &= 0xFF; Ram.memory[0x100 + SP] = PC / 256;  SP--;SP &= 0xFF; Ram.memory[0x100 + SP] = ST; SP--;SP &= 0xFF;
				PC = Ram.memory[0xFFFE] + Ram.memory[0xFFFF] * 256 - 1; cycles = 7; break; //BRK
			case 0x20: if (IR & 0xF) { ST &= 0x3D; ST |= (Ram.memory[addr] & 0xC0) | (!(A&Ram.memory[addr])) << 1; } //BIT
					   else { Ram.memory[0x100 + SP] = (PC + 2) % 256; SP--;SP &= 0xFF; Ram.memory[0x100 + SP] = (PC + 2) / 256;  SP--;SP &= 0xFF; PC = Ram.memory[addr] + Ram.memory[addr + 1] * 256 - 1; cycles = 6; }  break; //JSR
			case 0x40: if (IR & 0xF) { PC = addr - 1; cycles = 3; } //JMP
					   else { if (SP >= 0xFF) return 0xFE; SP++;SP &= 0xFF; ST = Ram.memory[0x100 + SP]; SP++;SP &= 0xFF; T = Ram.memory[0x100 + SP]; SP++;SP &= 0xFF; PC = Ram.memory[0x100 + SP] + T * 256 - 1; cycles = 6; }  break; //RTI
			case 0x60: if (IR & 0xF) { PC = Ram.memory[addr] + Ram.memory[addr + 1] * 256 - 1; cycles = 5; } //JMP() (indirect)
					   else { if (SP >= 0xFF) return 0xFF; SP++;SP &= 0xFF; T = Ram.memory[0x100 + SP]; SP++;SP &= 0xFF; PC = Ram.memory[0x100 + SP] + T * 256 - 1; cycles = 6; }  break; //RTS
			case 0xC0: T = Y - Ram.memory[addr]; ST &= 124;ST |= (!(T & 0xFF)) << 1 | (T & 128) | (T >= 0); break; //CPY
			case 0xE0: T = X - Ram.memory[addr]; ST &= 124;ST |= (!(T & 0xFF)) << 1 | (T & 128) | (T >= 0); break; //CPX
			case 0xA0: Y = Ram.memory[addr]; ST &= 125;ST |= (!Y) << 1 | (Y & 128); break; //LDY
			case 0x80: Ram.memory[addr] = Y; storadd = addr;  //STY
			}
		}
	}
